...
main() {
    ...
    echo "* Upload results..."
    star_genome_bam=$(dx upload ${reads_fn}_star_genome.bam --details "{ $meta }" --property SW="$versions" --brief)
    star_anno_bam=$(dx upload ${reads_fn}_star_anno.bam     --details "{ $meta }" --property SW="$versions" --brief)
