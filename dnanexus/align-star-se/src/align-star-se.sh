...
main() {
    ...
    details=`echo { $meta }`
    star_genome_bam=$(dx upload ${reads_fn}_star_genome.bam --details "$details" --property QC="$meta" --property SW="$versions" --brief)
    star_anno_bam=$(dx upload ${reads_fn}_star_anno.bam     --details "$details" --property QC="$meta" --property SW="$versions" --brief)
    star_log=$(dx upload ${reads_fn}_star_Log.final.out --property SW="$versions" --brief)
