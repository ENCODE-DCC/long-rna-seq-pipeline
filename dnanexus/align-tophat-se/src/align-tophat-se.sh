...
main() {
    ...
    /usr/bin/tophat -p ${nthreads} -z0 -a 8 -m 0 --min-intron-length 20 --max-intron-length 1000000 \
       ...
       --library-type fr-unstranded -G ${index_prefix}.gff --transcriptome-index ${index_prefix} ${index_prefix} ${reads_fn}.fastq.gz

    ../resources/usr/bin/samtools view -H tophat_out/accepted_hits.bam > header.txt
    echo "@PG  ID:Bowtie   VN:2.1.0.0" >> header.txt
    echo "@PG ID:Samtools VN:0.1.17.0" >> header.txt
    echo "@CO ID:Gencode  VN:19" >> header.txt

    ../resources/usr/bin/samtools reheader header.txt tophat_out/accepted_hits.bam > tmp.bam
    mv tmp.bam tophat_out/accepted_hits.bam

    ../resources/usr/bin/samtools reheader header.txt tophat_out/unmapped.bam > tmp.bam
    ...
    ../resources/usr/bin/samtools index merged.bam merged.bai
